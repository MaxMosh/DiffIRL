...
from tools.diffusion_model_acados import ConditionalDiffusionModel
...
DATASET_PATH = "data/dataset_acados.pkl"
...
def prepare_scalers(dataset_path):
    ...
    # Save
    joblib.dump(scaler_w, os.path.join(CHECKPOINT_DIR, "scaler_w_acados.pkl"))
    joblib.dump(scaler_traj, os.path.join(CHECKPOINT_DIR, "scaler_traj_acados.pkl"))
...
def main():
    ...
    scaler_w_path = os.path.join(CHECKPOINT_DIR, "scaler_w_acados.pkl")
    scaler_traj_path = os.path.join(CHECKPOINT_DIR, "scaler_traj_acados.pkl")
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 1) % 500 == 0:
            torch.save(model.state_dict(), os.path.join(CHECKPOINT_DIR, f"diff_model_acados_transformer_epoch_{epoch+1}.pth"))

    torch.save(model.state_dict(), os.path.join(CHECKPOINT_DIR, "diff_model_acados_transformer_final.pth"))
